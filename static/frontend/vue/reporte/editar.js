...
new Vue({
    ...
    methods: {
        ...
        async initDatos(){
            await axios.get(api_url_editar, {params:{
                idreporte: let_idreporte
                }})
                .then(response => {
        ...
        async verificarFecha() {
            ...
            await axios.post(api_url_verificar_fecha, { fecha_reporte: this.fecha_reporte })
                .then(response => {
                    if (response.data.reporte_existe && self.fecha_reporte !== const_fecha_reporte) {
        ...
        async finalizar() {
            ...
            if (this.existeFecha) {
                swal({
                    title: "Advertencia!!",
                    text: `Ya existe un reporte con esa fecha. 
                    Si continúa el reporte de fecha ${const_fecha_reporte} será eliminado 
                    y los datos serán añadidos al reporte de fecha ${self.fecha_reporte}. ¿Desea continuar?`,
                    type: "warning",
        ...
        async guardar(acumular) {
            ...
            await axios.post(api_url_editar, datos_reporte)
                .then(res => {
                    // console.log(res.data)
                    let_idreporte = res.data.idreporte;
                    swal({
                        title: "Correcto!!",
                        text: "El reporte se ha editado correctamente. ¿Desea continuar?",
                        type: "success",
